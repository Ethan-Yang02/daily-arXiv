...
2. 3. 让你选择环境管理方式：Conda 或 venv。
4. 扫描已有环境，并让你选择“使用已有环境”或“创建新环境”。
5. 安装 `requirements.txt`。
6. 生成适配本机的 `/etc/systemd/system/daily-arxiv-scheduler.service`。
...
脚本会交互确认后执行：
- 停止 `daily-arxiv-scheduler`
- 禁用开机自启
- 删除 `/etc/systemd/system/` 下的 scheduler service 文件
